...
class Generator3D(object):
    ...
    def generate_mesh(self, data, return_stats=True):
        ...
        if self.vol_bound is not None:
            ...
            with torch.no_grad():
                ...
                c_quantized, _, _ = self.model.quantize(c)
    ...
    def generate_mesh_recon_baseline(self, data, return_stats=True):
        ''' Generates the output mesh.
        ...
        for i in range(c_quantized['xz'].shape[0]):
            quant_c = {'xz':c_quantized['xz'][i].unsqueeze(0),'xy':c_quantized['xy'][i].unsqueeze(0),'yz':c_quantized['yz'][i].unsqueeze(0)}
            mesh = self.generate_from_latent(quant_c, stats_dict=stats_dict, **kwargs)
            mesh_list.append(mesh)
        return mesh_list
    
    def generate_mesh_batch_metric(self, data, return_stats=True):
        ''' Generates the output mesh.
        ...
        return mesh_list, category_list, name_list
    ...
    def generate_from_latent_interpolate(self, c_quantize=None, stats_dict={}, **kwargs):
        ...
        '''
        threshold = np.log(self.threshold) - np.log(1. - self.threshold) # logit: 10 --> Softmax: 0.4    0.4: ----> Inverse Softmax: 10
        ...
        while points.shape[0] != 0:
            ...
            values = self.eval_points_interpolate(pointsf, c_quantize, **kwargs).cpu().numpy()
        ...
        mesh = self.extract_mesh(value_grid, c_quantize, stats_dict=stats_dict)
        return mesh


    def eval_points_interpolate(self, p, c_quantize=None, vol_bound=None, **kwargs):
        ...
        for pi in p_split:
            ...
            with torch.no_grad():
                # c_quantized, _, _ = self.model.quantize(c)
                occ_hat = self.model.decode(pi, c_quantize, **kwargs).logits
                # occ_hat = self.model.decode(pi, c, **kwargs).logits
            occ_hats.append(occ_hat.squeeze(0).detach().cpu())
        occ_hat = torch.cat(occ_hats, dim=0)
        return occ_hat

    def generate_from_latent_revise(self, c=None, stats_dict={}, **kwargs):
        ''' Generates mesh from latent.
            Works for shapes normalized to a unit cube
        ...
        mesh = self.extract_mesh_revise(value_grid, c, stats_dict=stats_dict)
        return mesh
    
    def extract_mesh_revise(self, occ_hat, c=None, stats_dict=dict()):
        ...
        verts, faces, normals, values = measure.marching_cubes_lewiner(occ_hat, threshold)
        mesh = trimesh.Trimesh(vertices=verts/occ_hat.shape[-1] - 0.5, faces=faces)
