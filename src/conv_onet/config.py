...
def get_model(cfg, device=None, dataset=None, **kwargs):
    ...
    """Revise: Add Quantizer Start"""
    quantizer = cfg['model']['quantizer']       # multi or single
    print('Using Quantizer: {}'.format(quantizer))
    ...
    quantizer = models.quantizer_dict[quantizer](cfg=cfg, n_e=embed_num,e_dim=embed_dim,beta=beta,c_dim=c_dim,unet_kwargs=unet2d_kwargs)
    """Revise: Add Quantizer End"""
...
def get_model_improve(cfg, device=None, dataset=None, **kwargs):
    ''' Return the Occupancy Network model.
    ...
    # unet3d_kwargs = cfg['model']['encoder_kwargs']['unet3d_kwargs']
    unet2d_kwargs = cfg['model']['encoder_kwargs']['unet_kwargs']
    quantizer = models.quantizer_dict[quantizer](cfg, n_e=embed_num,e_dim=embed_dim,beta=beta,c_dim=c_dim,unet_kwargs=unet2d_kwargs)
    """Revise: Add Quantizer End"""
    ...
    # decoder = nn.DataParallel(decoder,device_ids=range(2))
    # encoder = nn.DataParallel(encoder,device_ids=range(2))
    # quantizer = nn.DataParallel(quantizer,device_ids=range(2))
    model = models.ConvolutionalOccupancyNetwork(
        decoder, quantizer, encoder, device=device
    )
