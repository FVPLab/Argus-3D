...
class Generator3D_Stage2(object):
    ...
    def generate_mesh_stage2(self, data, top_k=250, return_stats=True):
        ...
        # c_quantized = self.model.batch_predict_autoregressive(bs,device,top_k=top_k)    # B x C x reso x reso x reso
        # c_quantized = self.model.batch_predict_directly(shape_img)            # B x C x reso x reso x reso
        # c_quantized = self.model.batch_test_3plane(bs, device,plane_type='all')
        # c_quantized = self.model.batch_test_64_index(bs, device)


        # c_quantized = self.model.batch_test_32_index(bs,device)
        c_quantized = self.model.batch_test_32_index_quantize_single(bs,device)


        # c_quantized = self.model.batch_test_3plane(bs, device, plane_type='all')
        # c_quantized = self.model.batch_test_3plane_single(bs,device,plane_type='all')
        # c_quantized = self.model.batch_test_3plane_partial(bs, device, 'yz')
    ...
    def generate_mesh_stage2_reso32(self, data, top_k=250, return_stats=True):
        ''' Generates the output mesh.
        ...
        '''Autoregressive generate Feature Grid c'''
        c_quantized = self.model.batch_predict_reso32(bs, device)
        stats_dict['time (Autoregressive Generate c)'] = time.time() - t0
    ...
    def generate_mesh_stage2_create_quantize(self, data, top_k=250, return_stats=True):
        ''' Generates the output mesh.
        ...
        plane_xz, plane_xy, plane_yz = self.model.batch_predict_reso32_cond_get_quantize(data, device)
        stats_dict['time (Autoregressive Generate c)'] = time.time() - t0
        mesh_list = []
    ...
    def generate_mesh_stage2_reso32_cond(self, bs, top_k=250, return_stats=True,greed=False):
        ''' Generates the output mesh.
        ...
        print('greed:',greed)
        '''Autoregressive generate Feature Grid c'''
        # c_quantized = self.model.batch_predict_reso32_cond(bs, device)
        plane_xz, plane_xy, plane_yz = self.model.batch_predict_reso32_cond(bs, device,top_k=top_k)
        stats_dict['time (Autoregressive Generate c)'] = time.time() - t0
        mesh_list = []
        for i in range(plane_xz.shape[0]):
            c_quantized = {'xz':plane_xz[i].unsqueeze(0),'xy':plane_xy[i].unsqueeze(0),'yz':plane_yz[i].unsqueeze(0)}
    ...
    def generate_mesh_stage2_reso64_cond(self, bs, top_k=250, return_stats=True,greed=False):
        ''' Generates the output mesh.
        ...
        '''Autoregressive generate Feature Grid c'''
        # c_quantized = self.model.batch_predict_reso32_cond(bs, device)
        plane_xz, plane_xy, plane_yz = self.model.batch_predict_reso64_cond(bs, device)
        stats_dict['time (Autoregressive Generate c)'] = time.time() - t0
        mesh_list = []
        for i in range(plane_xz.shape[0]):
            c_quantized = {'xz':plane_xz[i].unsqueeze(0),'xy':plane_xy[i].unsqueeze(0),'yz':plane_yz[i].unsqueeze(0)}
    ...
    def generate_mesh_stage2_reso16_cond(self, bs, top_k=250, return_stats=True,greed=False):
        ''' Generates the output mesh.
        ...
        '''Autoregressive generate Feature Grid c'''
        # c_quantized = self.model.batch_predict_reso32_cond(bs, device)
        plane_xz, plane_xy, plane_yz = self.model.batch_predict_reso16_cond(bs, device,top_k=top_k)
        stats_dict['time (Autoregressive Generate c)'] = time.time() - t0
        mesh_list = []
        for i in range(plane_xz.shape[0]):
            c_quantized = {'xz':plane_xz[i].unsqueeze(0),'xy':plane_xy[i].unsqueeze(0),'yz':plane_yz[i].unsqueeze(0)}
    ...
    def generate_mesh_stage2_reso32_cond_class(self, bs, top_k=250, return_stats=True, class_idx=None):
        ''' Generates the output mesh.
        ...
        plane_xz, plane_xy, plane_yz = self.model.batch_predict_reso32_cond_class(bs, device,top_k=top_k, class_idx=class_idx)
        stats_dict['time (Autoregressive Generate c)'] = time.time() - t0
        mesh_list = []
        for i in range(plane_xz.shape[0]):
            c_quantized = {'xz':plane_xz[i].unsqueeze(0),'xy':plane_xy[i].unsqueeze(0),'yz':plane_yz[i].unsqueeze(0)}
    ...
    def generate_mesh_stage2_reso32_cond_class_weight_optim(self, bs, top_k=250, return_stats=True, weight=None):
        ''' Generates the output mesh.
        ...
        '''Autoregressive generate Feature Grid c'''
        # c_quantized = self.model.batch_predict_reso32_cond(bs, device)
        plane_xz, plane_xy, plane_yz = self.model.batch_predict_reso32_cond_class_weight_optim(bs, device,top_k=top_k, weight=weight)
        stats_dict['time (Autoregressive Generate c)'] = time.time() - t0
        mesh_list = []
        for i in range(plane_xz.shape[0]):
            c_quantized = {'xz':plane_xz[i].unsqueeze(0),'xy':plane_xy[i].unsqueeze(0),'yz':plane_yz[i].unsqueeze(0)}
    ...
    def generate_mesh_batch_test(self, data, return_stats=True):
        ''' Generates the output mesh.
        ...
        t0 = time.time()
        with torch.no_grad():
            c = self.model.Stage1_encode_inputs(data)
            c, _, _ = self.model.Stage1_quantize(c)
        mesh_list = []
        for i in range(c['xz'].shape[0]):
            c_quantized = {'xz':c['xz'][i].unsqueeze(0),'xy':c['xy'][i].unsqueeze(0),'yz':c['yz'][i].unsqueeze(0)}
            mesh = self.generate_from_latent(c_quantized, stats_dict=stats_dict, **kwargs)
            mesh_list.append(mesh)
        
        return mesh_list

    def generate_mesh_stage2_reso32_cond_class_inter(self, bs, top_k=250, return_stats=True, class_idx1=None,class_idx2=None):
        ''' Generates the output mesh.
        ...
        '''Autoregressive generate Feature Grid c'''
        class_idx1 = class_idx1.to(device)
        class_idx2 = class_idx2.to(device)
        # c_quantized = self.model.batch_predict_reso32_cond(bs, device)
        plane_xz, plane_xy, plane_yz = self.model.batch_predict_reso32_cond_class_inter(bs, device,top_k=top_k, class_idx1=class_idx1, class_idx2=class_idx2)
        stats_dict['time (Autoregressive Generate c)'] = time.time() - t0
        mesh_list = []
        for i in range(plane_xz.shape[0]):
            c_quantized = {'xz':plane_xz[i].unsqueeze(0),'xy':plane_xy[i].unsqueeze(0),'yz':plane_yz[i].unsqueeze(0)}
    ...
    def generate_mesh_stage2_reso32_cond_64(self, bs, top_k=250, return_stats=True):
        ''' Generates the output mesh.
    ...
    def eval_points(self, p, c_quantized=None, vol_bound=None, **kwargs):
        ...
        for pi in p_split:
            if self.input_type == 'pointcloud_crop':
                ...
                occ_hat = self.model.Stage1_decode(pi, c_quantized, **kwargs).logits
