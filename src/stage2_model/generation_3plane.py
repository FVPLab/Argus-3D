...
class Generator3D_Stage2(object):
    ...
    def generate_mesh_stage2_no_data(self,top_k, return_stats=True):
        ...
        print('Using Generation 3Plane !!!!')
    
        c_quantized = self.model.batch_predict_autoregressive_3plane_cond(bs,device,top_k=top_k)    # B x C x reso x reso x reso
        # c_quantized = self.model.batch_test_3plane_partial(bs,device,plane_type='all')
        # c_quantized = self.model.batch_predict_directly(shape_img)            # B x C x reso x reso x reso
        # c_quantized = self.model.batch_test(bs,device)
      
        # c_quantized = self.model.batch_test_3plane(bs, device, 'all')
        # c_quantized = self.model.batch_test_3plane_partial(bs, device, 'xz')
    ...
    def generate_mesh_stage2_no_data_batch(self,top_k, return_stats=True,bs=1):
        ''' Generates the output mesh.
        ...
        # input the entire volume
        t0 = time.time()
        print('Using Batch Generation 3Plane !!!!')
        plane_xz, plane_xy, plane_yz = self.model.batch_predict_autoregressive_3plane_cond_batch(bs,device,top_k=top_k)    # B x C x reso x reso x reso
        stats_dict['time (Autoregressive Generate c)'] = time.time() - t0
        ...
        if return_stats:
            return mesh_list, stats_dict
        else:
            return mesh_list
    

    def generate_mesh_stage2_no_data_class_condition(self,top_k, return_stats=True,class_idx=None):
        ''' Generates the output mesh.
        ...
        c_quantized = self.model.batch_predict_autoregressive_3plane_cond(bs,device,top_k=top_k, class_idx=class_idx)    # B x C x reso x reso x reso
    ...
    def generate_mesh_stage2_no_data_order1(self,top_k, return_stats=True):
        ''' Generates the output mesh.
        ...
        c_quantized = self.model.batch_predict_autoregressive_3plane_cond_order1(bs,device,top_k=top_k)    # B x C x reso x reso x reso
    ...
    def generate_mesh_stage2_no_data_order2(self,top_k, return_stats=True):
        ''' Generates the output mesh.
        ...
        c_quantized = self.model.batch_predict_autoregressive_3plane_cond_order2(bs,device,top_k=top_k)    # B x C x reso x reso x reso
    ...
    def generate_mesh_stage2(self, data,top_k, return_stats=True):
        ''' Generates the output mesh.
    ...
    def generate_mesh_stage2_noise(self, data,top_k, return_stats=True,noise=None):
        ''' Generates the output mesh.
        ...
        c_quantized = self.model.batch_predict_autoregressive_3plane_cond_noise_interpolation(bs,device,top_k=top_k,noise=noise)    # B x C x reso x reso x reso
        # c_quantized = self.model.batch_predict_directly(shape_img)            # B x C x reso x reso x reso
        # c_quantized = self.model.batch_test(bs,device)
