...
class Shapes3dDataset(data.Dataset):
    ...
    def __init__(self, dataset_folder, fields, split=None,
        ...
        # Set index
        print('/src/data/core.py, l-79', categories)
        ...
        for c_idx, c in enumerate(categories):
            ...
            if split is None:
                ...
                with open(split_file,'r') as f:
                    models_c_train = f.read().split('\n')
                if '' in models_c_train:
                    models_c_train.remove('')
                
                # val.lst
                split_file = os.path.join(subpath,'val.lst')
                with open(split_file,'r') as f:
                    models_c_val = f.read().split('\n')
                if '' in models_c_val:
                    models_c_val.remove('')
                models_c = models_c_train + models_c_val        # train + val 80%
                self.models += [
                    {'category': c, 'model': m}
                    for m in models_c
                ]
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:      # data: {'points','points.occ','inputs','inputs.normals'}
...
class Shapes3dDataset_subset(data.Dataset):
    ''' 3D Shapes dataset class.
    '''
    ...
    def __init__(self, dataset_folder, fields, split=None,
        ...
        self.metric_test_folder = cfg['data']['metric_test_folder']
        ...
        # Set index
        print('/src/data/core.py, l-346', categories)
        for c_idx, c in enumerate(categories):
            self.metadata[c]['idx'] = c_idx
        ...
        print('Split:',split)
        for c_idx, c in enumerate(categories):
            subpath = os.path.join(self.metric_test_folder, c)
            if not os.path.isdir(subpath):
                logger.warning('Category %s does not exist in dataset.' % c)

            if split is None:
                self.models += [
                    {'category': c, 'model': m} for m in [d for d in os.listdir(subpath) if  d != '' ]
                ]
    ...
    def __getitem__(self, idx):
        ...
        model_path = os.path.join(self.metric_test_folder, category, model)
        data = {}
