...
class LocalPoolPointnet(nn.Module):
    ...
    def generate_plane_features(self, p, c, plane='xz'):
        ...
        fea_plane = c.new_zeros(p.size(0), self.c_dim, self.reso_plane**2)  # b x 32 x 1024
        c = c.permute(0, 2, 1) # B x 512 x T
        fea_plane = scatter_mean(c, index, out=fea_plane) # B x 512 x reso^2  index: 3000  c: b x 32 x 3000
    ...
    def pool_local(self, xy, index, c):
        ...
        for key in keys:
            ...
            fea = fea.gather(dim=2, index=index[key].expand(-1, fea_dim, -1))   # b x 32 x 3000
            c_out += fea        
        return c_out.permute(0, 2, 1)


    def forward(self, p):
        batch_size, T, D = p.size()#[B, 30000, 3]

        # acquire the index for each point
        coord = {}
        index = {}
        if 'xz' in self.plane_type:
            coord['xz'] = normalize_coordinate(p.clone(), plane='xz', padding=self.padding)#[B, 30000, 2]
            index['xz'] = coordinate2index(coord['xz'], self.reso_plane)#[B, 1, 30000]
        ...
        net = self.fc_pos(p)    # b x 3000 x 64 [B, 30000, 64]

        net = self.blocks[0](net)   # b x 3000 x 32 [B, 30000, 32]
        for block in self.blocks[1:]:
            pooled = self.pool_local(coord, index, net)
            net = torch.cat([net, pooled], dim=2)
            net = block(net)
        # print(net.shape) #[B, 30000, 32]
        c = self.fc_c(net)  # b x 3000 x 32 --> b x 3000 x 32 #[B, 30000, 32]

        fea = {}
        if 'grid' in self.plane_type:
            fea['grid'] = self.generate_grid_features(p, c)
        if 'xz' in self.plane_type:
            fea['xz'] = self.generate_plane_features(p, c, plane='xz')#[B, 32, res, res]
        if 'xy' in self.plane_type:
